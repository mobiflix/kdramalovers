...
// ============================================================
// POPULATE COUNTRY DROPDOWNS (Korean Only)
// ============================================================
...
// 🔥 Top 20 KDrama Today (trending sa Korea ngayon)
async function fetchTop20KDramaToday(page) {
...
// 📺 Most Watched KDrama (popularity desc)
async function fetchMostWatchedKDrama(page) {
  const today = new Date().toISOString().split('T')[0];
...
// ⭐ Highest Rated KDrama
async function fetchHighestRatedKDrama(page) {
  const today = new Date().toISOString().split('T')[0];
  const url = `${BASE_URL}/discover/tv?api_key=${API_KEY}` +
    `&with_origin_country=${KOREAN_COUNTRY}` +
    `&with_original_language=${KOREAN_LANG}` +
...
// 🆕 Newest KDrama (present year pababa)
async function fetchNewestKDrama(page) {
  const today = new Date().toISOString().split('T')[0];
  const url = `${BASE_URL}/discover/tv?api_key=${API_KEY}` +
    `&with_origin_country=${KOREAN_COUNTRY}` +
    `&with_original_language=${KOREAN_LANG}` +
...
// ✅ Completed KDrama
async function fetchCompletedKDrama(page) {
  const url = `${BASE_URL}/discover/tv?api_key=${API_KEY}` +
    `&with_origin_country=${KOREAN_COUNTRY}` +
    `&with_original_language=${KOREAN_LANG}` +
    `&sort_by=popularity.desc` +
...
async function generateNotifications() {
  ...
  try {
    ...
    const [newTVRes, ongoingRes] = await Promise.all([
      fetch(`${BASE_URL}/discover/tv?api_key=${API_KEY}&sort_by=first_air_date.desc&first_air_date.gte=${weekAgo}&first_air_date.lte=${today}&vote_count.gte=5&with_origin_country=${KOREAN_COUNTRY}&with_original_language=${KOREAN_LANG}`),
      fetch(`${BASE_URL}/discover/tv?api_key=${API_KEY}&sort_by=popularity.desc&first_air_date.lte=${today}&vote_count.gte=20&with_status=0|1&with_origin_country=${KOREAN_COUNTRY}&with_original_language=${KOREAN_LANG}&page=1`)
    ]);
    ...
    (newTV.results || []).slice(0, 8).forEach(function(item) {
      if (!item.poster_path) return;
      notifications.push({
        id: item.id, media_type: 'tv', title: item.name, poster_path: item.poster_path,
        type: 'new_release', message: 'New KDrama released!', createdAt: Date.now(), read: false
      });
    });
...
function playEpisode(tvId, seasonNumber, episodeNumber) {
  ...
  const title = currentItem ? (currentItem.title || currentItem.name) : 'KDrama';
...
function displayBanner(item) {
  ...
  if (typeEl) {
    const type = item.media_type === 'movie' ? 'Movie' : 'KDrama';
...
async function loadOngoingPageBatch() {
  ...
  try {
    const data = await fetchOngoingKDrama(ongoingPageState.page);
...
async function loadCompletedPageBatch() {
  ...
  try {
    const data = await fetchCompletedKDrama(completedPageState.page);
...
function openViewAll(key) {
  ...
  const configs = {
    ...
    most_watched: { name: 'Most Watched KDrama',  icon: '🔥', media: 'tv',    type: 'most_watched' },
    highest:      { name: 'Highest Rated KDrama', icon: '⭐', media: 'tv',    type: 'highest' },
    newest:       { name: 'Newest KDrama',        icon: '🆕', media: 'tv',    type: 'newest' },
    ongoing:      { name: 'Ongoing KDrama',       icon: '🔴', media: 'tv',    type: 'ongoing' },
    completed:    { name: 'Completed KDrama',     icon: '✅', media: 'tv',    type: 'completed' }
  };
...
async function loadViewAllBatch() {
  ...
  try {
    ...
    if (hasFilters) {
      ...
      else if (config.type === 'newest_tv') data = await fetchNewestKDrama(viewAllState.page);
      else if (config.type === 'most_watched') data = await fetchMostWatchedKDrama(viewAllState.page);
      else if (config.type === 'highest') data = await fetchHighestRatedKDrama(viewAllState.page);
      else if (config.type === 'newest') data = await fetchNewestKDrama(viewAllState.page);
      else if (config.type === 'ongoing') data = await fetchOngoingKDrama(viewAllState.page);
      else if (config.type === 'completed') data = await fetchCompletedKDrama(viewAllState.page);
    }
...
async function init() {
  try {
    ...
    const [top20Data, mostWatchedData, highestData, newestData, ongoingData, completedData, newestMoviesData] = await Promise.all([
      fetchTop20KDramaToday(1),
      fetchMostWatchedKDrama(1),
      fetchHighestRatedKDrama(1),
      fetchNewestKDrama(1),
      fetchOngoingKDrama(1),
      fetchCompletedKDrama(1),
...
function openVivamaxPage() { openViewAll('newest'); }
function closeVivamaxPage() { closeViewAll(); }
function openMoviesPage() { openViewAll('movies'); }
function openSeriesPage() { openViewAll('tv'); }
function closeMoviesPage() { closeViewAll(); }
function closeSeriesPage() { closeViewAll(); }