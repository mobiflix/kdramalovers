...
function renderNotifications() {
  ...
  list.forEach(function(notif) {
    ...
    item.onclick = function() {
      closeNotifications();
      window.location.href = `details.html?id=${notif.id}&type=${notif.media_type}`;
    };
    const img = document.createElement('img');
...
function closeAllPagesOnly() {
  ['view-all-page', 'provider-page', 'my-list-page', 'more-page', 'search-modal', 'genre-page',
   'user-profile-page', 'person-page'].forEach(function(id) {
...
function showDetails(item) {
  ...
  if (!mediaType) {
    if (item.first_air_date || (!item.title && item.name)) mediaType = 'tv';
...
function renderMyList() {
  let list = [];
  try { list = JSON.parse(localStorage.getItem('mobiflix_watchlist')) || []; }
  catch (e) {}
...
async function init() {
  try {
    ...
    const [top20Data, mostWatchedData, highestData, newestData, newestMoviesData] = await Promise.all([
